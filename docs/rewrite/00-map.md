...
**Verdict: reshape.** A match records scores, scorers, assists, own goals, a penalty shoot-out for a one-leg Knockout match, a lineup per side with a chosen formation, and a video link. Goals reconcile with the score. No cards, minutes or substitution events; a lineup may carry more players than the match format, the rest being substitutes (settled in `03-competitions.md`).
