...
- **Match format (five-a-side, six-a-side and so on) lives at two levels.** In Pickup it is a fact of each match, chosen when the match is added and prefilled from the competition's usual match format. In League and Knockout it is a competition setting, because a schedule and its rosters are built for one size; the competitions session lets it change only while the Current season has no matches.
- **A draw is a result** in Pickup and League. Penalties exist only in a one-leg Knockout match, where they decide the winner. Amended in `07-knockout.md`: in the deciding match of a Knockout Tie, which with two legs is the second leg.
...
Terms today's code uses without defining, to be settled in the sessions named: **side** (matches), **round** and **matchday** (league schedule), **ballot**, **vote**, **points** and **rating** (voting, settled in `09-voting.md`: Ballot, Vote, Pick and Rating, with points confined to the arithmetic), **pending votes** (voting, today both a roster and a count; settled in `09-voting.md` as Turnout for the match and Your open votes for the Account), **registered** (identity, today two meanings), **match type** (matches, today "format" and validated nowhere).
...
| Rule                                                                                                                                                                                                                                                                                                                                                                     | Owner        | Status                                                                                                                                                                                                                                                                                                                           |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Completed match is the unit that counts** for standings, stats and Minimum matches. A Pickup match is born completed; a Fixture is completed by an act.                                                                                                                                                                                                                | Matches      | Kept, and reshaped in `08-matches.md`: every match of every Format, a Pickup match included, is to play until the Complete act; a Pickup match is no longer born completed.                                                                                                                                                      |
| **Every new match lands in the Current season.** A Past season is read-only for match writes. Voting open at rollover runs to its deadline.                                                                                                                                                                                                                              | Seasons      | Kept, by purpose this time: closing a Season needs one Completed match and turns its unfinished Fixtures into Not played. Stated in `04-seasons.md` with its two exceptions: a Group admin may make a Correction of a Past season match's result facts, and Reopen last season while nothing has been played since.              |
...
| **Only a list that grows without bound is paged.** A derived table (the player table, Standings, Records) is never paged, because ranking one row needs the whole scope; a list bounded by its structure, the Player pool included, is loaded whole; the append-only records, past invitations and the two match histories are paged by the server and end in Show more. | Architecture | New, added 2026-09-17 after the branch sessions. Stated in `12-architecture.md` as Paged reads: a keyset cursor, fifty rows fixed by the server, no totals, filters as inputs of the read; its screen half in `13-frontend.md`.                                                                                                  |
...
**Verdict: reshape.** Three formats under one Competition concept: **Pickup** (squad format), **League** and **Knockout** (team formats). Settings editable after creation, applied forward. Voting is a per-competition switch with its own settings. Match format is a competition setting for League and Knockout, changeable only while the Current season has no matches; a Pickup competition carries only a usual match format that prefills each match, because each Pickup match picks its own. Settled in `03-competitions.md`, which also replaces archive and delete with End competition, a Group admin act that closes the Current season without opening the next.
...
**Verdict: keep, reshaped.** Seasons for every format. Read-only Past seasons stay, now by purpose: rollover happens when the previous matches are settled. Reset is dropped. Settled in `04-seasons.md`: closing needs one Completed match, unfinished Fixtures become Not played, a Group admin may make a Correction of a Past season match's result facts, Reopen last season undoes a rollover or an End while nothing has been played since, and a Season carries an optional label and a derived summary.
...
**Verdict: reshape.** Rosters become real: a team format match's players come from the two teams' rosters. Roster setup is a per-Season act for team formats and is separate from generating a schedule. Settled in `05-teams.md`: a Team is a Group record that enters any League or Knockout of the Group through an Entry per Season, each Entry holding its roster; rosters are exclusive within a Season with a Guest as the recorded exception; name and colour only; no roster size bounds; free mid-season changes, recorded.
...
**Verdict: keep, reshaped.** Standings are always derived from completed matches, never counted. Schedule generation is its own act. Settled in `06-league.md`: Rounds, not matchdays, with an optional planned date per Fixture; single or double round robin plus Add a cycle; extra Fixtures, Mark Not played, Walkover, and adding or withdrawing a team after generation; points, goal difference, goals for, head-to-head, then a shared position.
...
**Verdict: keep, built from nothing.** A team format sharing teams, rosters and match recording with League. Settled in `07-knockout.md`: any count from two with byes to the next power of two; a random draw or an ordered list of seeds with helpers from a League table or the previous cup; one or two legs with the Final one leg; Tie as the bracket node with every Fixture from the Draw; a shoot-out of two totals on the deciding match; a bye advances and covers every gap; Walkover, Not played, withdraw and a late entrant into a bye slot; an optional third-place match; an Entry's Result and an All seasons record.

**Depends on.** Teams and rosters; Seasons; a draw is a result, so the deciding match of a Knockout Tie is the only match penalties decide; Completed match.
...
- Whether a Knockout Season ends automatically when the final is completed. Settled in `03-competitions.md`: End competition is always the admin's act, and the Knockout page suggests it once the final is completed.
...
**Verdict: reshape.** A match records scores, scorers, assists, own goals, a shoot-out for the deciding match of a Knockout Tie (amended in `07-knockout.md` from "a one-leg Knockout match"), a lineup per side with a chosen formation, and a video link. Goals reconcile with the score. No cards, minutes or substitution events; a lineup may carry more players than the match format, the rest being substitutes (settled in `03-competitions.md`). Settled in `08-matches.md`: one life cycle for every Format, to play, Completed and Not played, so a Pickup match is no longer born completed; one date; a colour per Pickup side; counts per lineup row with own goals as a per-player fact and unattributed goals allowed; a fixed formation list with position as presentation only; result facts never reopen the vote and only adding or removing a Player does, the lineup freezing at the deadline; Un-complete; an eight-point server validation list; a per-match change record.
...
**Verdict: keep, reshaped.** Same ballot shape and shared crown. Ratings and crowns derived at read time. Reopen only on lineup change before the deadline. A Player casts their own ballot through a linked Account; an admin or manager may submit one on behalf of any Player in the match, linked or not. Minimum matches replaces the gate, Pickup only. Settled in `09-voting.md`: a ballot on behalf marked with the acting Account, replaceable by the Player and never overriding their own; ballots replaceable while open; points per ballot cast, 0.00 to 3.00, with a three-ballot floor below which a match is Not rated; closing on the last ballot of every voter, linked or not, or at the deadline, with Close now and Extend bounded to 14 days from completion; a full crown each on a tie; one vote shape across Formats; no vote under four Players; a secret ballot with a visible turnout and results only after close; a results breakdown, Your open votes, a voting record, one share link per match.
...
- Whether the rating denominator should be ballots cast (today it is vote rows, so an absent voter lowers everyone), and whether a match with few ballots should be rated at all. Corrected in `09-voting.md`: today's vote rows divided by three are the ballots cast, so an absent voter lowers nobody; the denominator stays ballots cast and a match needs three ballots to be Rated.
...
**Verdict: keep, reshaped.** Everything stays and gains a rating history over time. Group-scoped, not public. A draw counts as a draw everywhere; a Knockout penalty decision counts as a win and a loss. Settled in `10-stats.md`: the career is per Group with a private Your career across Groups; win rate is the points share; one player table in every scope with the Players page as the Group's leaderboard; an Award threshold per competition decides who is Qualified for a Season's MVP, Honours and Records; rating history, Form, streaks, clean sheets, attendance, played with and against; the Team page list; the Group home; Copy as text and Export CSV.
...
**Verdict: reshape.** A share link to post in the group chat is the primary path to a vote. Email stays for account matters and as an opt-in reminder. Settled in `11-notifications.md`: two classes of mail, Account mail and Notice; no in-app inbox; one Notice, the Vote reminder, off by default, offered once by the Reminder prompt, sent within 24 hours of the deadline, once per deadline; Copy reminder text from the Turnout; a Delivery per message; bounces and complaints mark an email Undeliverable; one-click unsubscribe on every Notice; push and the deferred hooks later.

**Depends on.** Account link; voting open and close rules; operations for scheduled work.

**Open questions.**

- Per-Account notification preferences: which emails, how often. Settled in `11-notifications.md`: one preference, the Vote reminder, off by default and offered once by the Reminder prompt; Account mail has none.
- Whether reminders are per competition settings or per Account. Settled in `03-competitions.md` and `11-notifications.md`: per Account, one switch across Groups.
- Push notifications or a PWA install prompt as a later channel. Settled in `11-notifications.md`: later, as a second channel under the same preference; the install prompt is the frontend session's.
- Deliverability duties: unsubscribe, bounces, complaints. Settled in `11-notifications.md`: one-click unsubscribe on every Notice, an Undeliverable mark from bounces and complaints, a Delivery per message.
