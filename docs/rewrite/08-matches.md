...
| Note                | result | Optional text of up to 500 characters: "played in the rain", "Marko's debut", "abandoned at sixty minutes, result stands"                                                                                                                                                                |
...
- **Pickup**: each side's picker offers the active pool with search and the inline nickname path; no guest concept. Two helpers fill the form: **Copy last lineup** prefills both sides from the competition's previous Completed match, since the regulars are the regulars, and **Draw sides** takes the Players who turned up and deals them across the two sides at random or balanced by average rating, the teams session's Draw teams reused, for adjustment before Save (the balance formula is settled in `10-stats.md`: the last 10 Rated matches Group-wide, the median for a Player with none, sums shown). The match format is prefilled from the usual match format and the side colours from the previous match.
