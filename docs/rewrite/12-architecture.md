...
Added on 2026-09-17, after the branch sessions, when pagination was found to be settled nowhere. The rule is a split. **A derived table is never paged**: the player table in every scope, Standings, Records, Honours, the Season summary and Your career need every Completed match of their scope to rank one row, so a page would save nothing; the read returns every row, the client sorts, filters and applies column presets over all of them, and Copy as text and Export CSV always carry the whole scope. **A list bounded by its structure is loaded whole**: the Schedule and the Bracket, whose Rounds are their pages, Teams, Members, Competitions, pending invitations, join links, an Account's sessions, Your open votes, and the Player pool, which a Group of 63 Players sends in a few kilobytes and the picker searches in the client, so there is no search procedure. **A list that grows without bound is paged by the server**: the four append-only records (the activity record, a match's change record, security events, Deliveries), past invitations (accepted, expired, revoked), and the two match histories, a competition's Completed matches and a Player's match history.

Paged reads share one shape: the input carries an optional `cursor` beside the read's filters, the output is `{ items, nextCursor }`, and `nextCursor` is null on the last page. The page is **50 rows, fixed by the server**; the client sends no limit, and no read returns a total, so there is no "page 3 of 9". The cursor is keyset, never an offset: `(instant, id)` descending on the append-only records and past invitations, `(match date, recording order, id)` descending on a match history, the order the stats session already uses, and since every id is a UUIDv7 the last term settles ties in time order. An entry appended while someone scrolls therefore never repeats or skips a row. The cursor is opaque to the client, an encoded string `server` alone reads. **A filter on a paged list is an input of the read**, the activity record's kind, a history's competition or Season, and changing one starts again from the first page; no paged list has free-text search. A match history row carries the match's stored facts plus the rating and the crown, which the engine derives from that match's ballots alone, so a page of fifty costs the ballots of fifty matches, not the scope's. A Season-scoped Matches tab goes through the same read and in practice never reaches a second page.
...
| 0012 | Migrated Accounts are verified and have no Sign-in method (session 15)       |
...
- **Paged reads by keyset cursor** (added 2026-09-17): fifty rows fixed by the server, `{ items, nextCursor }`, no totals; only the append-only records, past invitations and the two match histories; derived tables and the Player pool whole; filters as inputs of the read.

### Declined, and why

- **Paging everything uniformly**, the player table included. Ranking one row needs the whole scope, so the server would compute the table and then slice it; it works against ADR 0002 for volumes that do not ask.
- **Offset and limit**, and **a total count**. An entry appended mid-scroll repeats a row, and a count is a second query for a number no screen shows.
- **A server search procedure for the Player pool.** A round trip per keystroke at the pitch, on the connection most likely to drop, for a list of a few kilobytes.
- **A page size chosen by the client.** One more input to validate for no screen that needs it.
