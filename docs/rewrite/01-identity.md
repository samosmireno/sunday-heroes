...
- one or two **sign-in methods**: Password (a hash) and Google (attached by Google's stable subject id, at most one per Account). A method may be detached, never the last one. Settled in `15-migration.md` (ADR 0012): an Account carried over by the data migration is verified and has no method until its first sign-in, when Google attaches or forgot password sets one;
...
- **Data migration (15)**: whether password hashes migrate; today's Accounts have no verified-email flag and no Google subject id, so the migration decides what to assume for each. Settled in `15-migration.md`: no hash migrates; every migrated Account is verified and has no Sign-in method until its first sign-in, Google attaching by subject id then and forgot password setting a password (ADR 0012).
