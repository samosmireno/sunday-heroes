...
export default function TeamInputStats({
  ...
  useEffect(() => {
    players.forEach((player, index) => {
      const playerIndex = index + offset;
      form.setValue(`matchPlayers.players.${playerIndex}.nickname`, player);
      form.setValue(`matchPlayers.players.${playerIndex}.position`, index + 1);

      const currentGoals = form.getValues(
        `matchPlayers.players.${playerIndex}.goals`,
      );
      const currentAssists = form.getValues(
        `matchPlayers.players.${playerIndex}.assists`,
      );

      if (currentGoals === undefined) {
        form.setValue(`matchPlayers.players.${playerIndex}.goals`, 0);
      }
      if (currentAssists === undefined) {
        form.setValue(`matchPlayers.players.${playerIndex}.assists`, 0);
      }
    });
  }, [form, offset, players]);
